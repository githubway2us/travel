...
def create_tables():
    ...
    # รายชื่อจังหวัดตามภูมิภาค
    regions = {
        'ภาคเหนือ': [
            'เชียงใหม่', 'เชียงราย', 'ลำปาง', 'ลำพูน', 'แม่ฮ่องสอน', 'น่าน',
            'พะเยา', 'แพร่', 'อุตรดิตถ์', 'กำแพงเพชร', 'ตาก', 'พิษณุโลก',
            'เพชรบูรณ์', 'สุโขทัย', 'พิจิตร', 'นครสวรรค์', 'อุทัยธานี'
        ],
        ...
        'ภาคตะวันตก': [
            'กาญจนบุรี', 'เพชรบุรี', 'ประจวบคีรีขันธ์', 'ราชบุรี'
        ],
        'ภาคใต้': [
    ...
    # เพิ่มจังหวัดลงในฐานข้อมูลถ้ายังไม่มี
    for region, province_names in regions.items():
        for province_name in province_names:
            # ตรวจสอบว่าจังหวัดมีอยู่ในฐานข้อมูลหรือไม่
            existing_province = Province.query.filter_by(name=province_name).first()
            if not existing_province:
                new_province = Province(name=province_name)
                db.session.add(new_province)
                print(f"เพิ่มจังหวัด: {province_name}")
    
    # บันทึกการเปลี่ยนแปลงลงในฐานข้อมูล
    try:
        db.session.commit()
        print("บันทึกจังหวัดลงในฐานข้อมูลสำเร็จ")
    except Exception as e:
        db.session.rollback()
        print(f"เกิดข้อผิดพลาดในการบันทึกจังหวัด: {str(e)}")
